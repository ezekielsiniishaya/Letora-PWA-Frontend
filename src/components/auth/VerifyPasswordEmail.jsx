...
export default function VerifyPasswordEmail() {
  ...
  // In VerifyResetCode component - update the handleVerify function
  const handleVerify = async () => {
    ...
    try {
      const result = await verifyPasswordResetCodeAPI(verificationCode);
      setSuccess(
        "Code verified successfully! You can now reset your password."
      );
      setHasVerified(true);

      // Navigate to reset password page with the verified token
      setTimeout(() => {
        navigate("/reset-password", {
          state: {
            email: userEmail,
            resetCode: result.resetToken || verificationCode, // Use the returned token or fallback
          },
        });
      }, 1500);
    } catch (err) {
  ...
  // Also update the auto-verify useEffect
  useEffect(() => {
    const verificationCode = code.join("");
        ...
        try {
          ...
          setTimeout(() => {
            navigate("/reset-password", {
              state: {
                email: userEmail,
                resetCode: result.resetToken || verificationCode,
              },
            });
          }, 1500);
        } catch (err) {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const result = await resendPasswordResetCodeAPI(userEmail);
      setSuccess(result.message || "Reset code sent!");
      setResendTimer(120);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const verify = async () => {
        ...
        try {
          await verifyPasswordResetCodeAPI(verificationCode);
          setSuccess(
            "Code verified successfully! You can now reset your password."
          );
          setHasVerified(true);
    ...
    <div className="flex flex-col items-center min-h-screen bg-[#F9F9F9] px-[20px]">
      ...
      <div className="w-full max-w-sm mt-[26px]">
        ...
        <p className="text-[14px] text-[#666666] mt-[4px]">
          We've sent a password reset code to {userEmail}. Enter the code below.
        </p>
