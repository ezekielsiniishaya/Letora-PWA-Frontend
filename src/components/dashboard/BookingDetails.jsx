...
export default function BookingDetails({ role = "guest" }) {
  ...
  const currentStatus = statusMap[getBookingStatus(booking)] || statusMap.ONGOING;
  const paymentBreakdown = booking ? calculatePaymentBreakdown(booking.totalPrice) : null;
      ...
      <div className="w-full max-w-md space-y-2 px-[21px] pb-[75px]">
        ...
        <div className="bg-white rounded-[5px] py-[10px] px-[6px] text-[13px] text-[#505050]">
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              <span>Duration</span>
              <span>{calculateDuration(booking.startDate, booking.endDate)}</span>
            </div>
          </div>
        </div>
