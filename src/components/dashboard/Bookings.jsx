...
export default function MyBooking({
  ...
  const [actionCompleted, setActionCompleted] = useState(false); // 🔹 Track disable state
  ...
  return (
    <div className="bg-white rounded-[5px] w-full h-[158px] pt-[10px] px-[10px] relative">
      <div
        ...
        onClick={() =>
          status !== "ongoing"
            ? navigate(`/bookings/${lodge.id}`, { state: { lodge, status } })
            : null
        }
      >
        {/* Apartment Image */}
      ...
      <div className="flex justify-center mt-4 space-x-2">
        ...
        {status === "completed" && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (actionCompleted) return; // 🔹 Prevent further clicks
            ...
            disabled={actionCompleted} // 🔹 Disable when done
      ...
      {showRating && (
        <RatingPopup
          onClose={() => {
            setShowRating(false);
            setActionCompleted(true); // 🔹 Disable after rating
          }}
        />
      )}
      ...
      {showHoldSuccess && (
        <ShowSuccess
          ...
          onClose={() => {
            ...
            setActionCompleted(true); // 🔹 Disable after holding
          }}
          height="auto"
        />
      )}
